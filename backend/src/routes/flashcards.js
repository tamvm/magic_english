...
router.put('/session/:sessionId/end', async (req, res) => {
  try {
    ...
    const { cardsStudied, newCards, reviewCards, totalTime, correctAnswers, totalAnswers } = req.body;
    const userId = req.user.id;

    const averageResponseTime = totalAnswers > 0 ? Math.round(totalTime * 1000 / totalAnswers) : 0;
...
router.get('/:cardId/quiz', async (req, res) => {
  try {
    ...
    if (existingQuestions.length === 0) {
      ...
      const questionTypes = questionType ? [questionType] : ['fill_blank', 'definition_choice'];

      try {
        const newQuestions = await quizService.generateQuizQuestions(word, questionTypes, 1);
...
router.post('/quiz/:questionId/answer', async (req, res) => {
  try {
    ...
    const newSuccessRate = question.usage_count > 0 ?
      (question.success_rate * question.usage_count + (isCorrect ? 1 : 0)) / newUsageCount :
      isCorrect ? 1 : 0;
...
router.get('/progress', async (req, res) => {
  try {
    ...
    res.json({
      progressByDay: Object.values(progressByDay).sort((a, b) => new Date(a.date) - new Date(b.date)),
...
async function updateUserStatistics(supabase, userId, rating, responseTime) {
  try {
    ...
    const newRetentionRate = ((stats.average_retention_rate * stats.total_reviews) + wasCorrect) / newTotalReviews;
    const newAvgResponseTime = stats.total_reviews > 0 ?
      Math.round(((stats.average_response_time * stats.total_reviews) + responseTime) / newTotalReviews) :
      responseTime;
    ...
    const wordsMastered = masteredError ? (stats.words_mastered || 0) : masteredCards.length;
